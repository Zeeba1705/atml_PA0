...
\usepackage[colorlinks=true, urlcolor=blue, linkcolor=black, citecolor=black]{hyperref}       % hyperlinks
...
% Note. For the workshop paper template, both \title{} and \workshoptitle{} are required, with the former indicating the paper title shown in the title and the latter indicating the workshop title displayed in the footnote. 
\title{From Transfer Learning to Multi-Modal Alignment: Experiments with CNNs, Vision Transformers, CLIP, and VAEs}
...
\begin{abstract}
  This report explores the mechanisms of several different deep learning architectures through a series of practical experiments involving ResNet-152, Vision Transformers, CLIP, and Variational Autoencoders. The first experiment involved using ResNet-152 to study how transfer learning, residual connections and feature hierarchies affect CNNs and their performance. Next, Vision Transformers and their patch-based attention mechanisms were analyzed, along with how they respond to missing information and how different representations can be used for classification. The final two experiments investigated CLIP's zero-shot capabilities and the modality gap between image and text embeddings, followed by a VAE implementation used to study latent representations, reconstruction quality, and generation on MNIST. Overall, the results highlight how model performance and generalization ability are affected by the choice of architecture, pretraining objectives, and learned representations.
\end{abstract}

\section*{Github Repository}

The complete code and report source files are available at
\href{https://github.com/Zeeba1705/atml_PA0}{ATML-PA0}.

\section{Introduction}

This study comprises four central experiments conducted across different fields of deep learning, with each experiment focusing on the inner workings, representations, and evaluation of a different class of models. The first concept explored is that of CNNs and transfer learning, using ResNet-152 to study feature hierarchies, feature transferability, residual connections, and their effects on classification accuracy. The architecture of CNNs proves to be a very interesting aspect of their mechanism, with the use of convolutional filters representing a significant step in the field of computer vision.

Moving a step forward, this study then looks into Vision Transformers, another incredible breakthrough in the field, as ViTs were among the first models to show that a pure transformer architecture could match or outperform CNNs on vision tasks when trained on sufficiently large datasets. In this study, attention maps based on ViT outputs were extensively analyzed, while additional experiments involving patch masking and alternative pooling strategies were used to study robustness and the representations learned by the model.

While CNNs and ViTs in this study focus on supervised learning, CLIP brings in the concept of contrastive learning, which deals with learning representations by pulling matching pairs closer together in embedding space while pushing mismatched pairs apart, rather than relying on explicit class labels. Experiments were conducted to study the mode of operation of zero-shot image classification using CLIP, and the relationship between image and text embeddings was further studied through the modality gap, followed by Procrustes alignment to investigate whether bringing the two embedding spaces closer together could improve classification performance.

Lastly, Variational Autoencoders were explored through an implementation trained on the MNIST dataset. The experiment focused on latent-space learning, reconstruction quality, and generation of new samples, while also examining how the reconstruction and KL-divergence objectives work together to learn a structured latent representation. 
